...
//colBtn.tag = indexPath.section + 100;
...
//    self.childArray = [NSMutableArray arrayWithCapacity:0];
    ...
    gmp.mederatorIndex = self.moderatorsId;
...
//        [self.childArray addObject:[[[postDic objectForKey:@"post"]  objectAtIndex:i] objectForKey:@"child"]];
    }
...
//    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(getPost:) name:@"MEDERATORPOST"  object:nil];
...

{
    ...
    if (!self.isLoading) { // 判断是否处于刷新状态，刷新中就不执行
        ...
        if ((height - scrollView.contentSize.height + scrollView.contentOffset.y) / height > 0.2) {
            ...
            GetMederatorPost *gmp = [[GetMederatorPost alloc] init];
            gmp.mederatorIndex = self.moderatorsId;
            [gmp getMederatorPost:self.requestTimes];
            

            NSLog(@"%d,,,,%@",[[gmp.mederatorPostDic objectForKey:@"post"] count],self.postDic);
            for (int i = 0; i < [[gmp.mederatorPostDic objectForKey:@"post"]count]; i ++) {
                [self.postIdArray addObject:[[[postDic objectForKey:@"post"]  objectAtIndex:i] objectForKey:@"id"]];
                [self.customerIdArray addObject:[[[gmp.mederatorPostDic objectForKey:@"post"]  objectAtIndex:i] objectForKey:@"customer_id"]];
                //    [self.customerNameArray addObject:[[[dic objectForKey:@"post"]  objectAtIndex:i] objectForKey:@"customerName"]];
                [self.customerPortraitArray addObject:[[[gmp.mederatorPostDic objectForKey:@"post"]  objectAtIndex:i] objectForKey:@"customerPortrait"]];
                [self.titleArray addObject:[[[gmp.mederatorPostDic objectForKey:@"post"]  objectAtIndex:i] objectForKey:@"title"]];
                [self.contentArray addObject:[[[gmp.mederatorPostDic objectForKey:@"post"]  objectAtIndex:i] objectForKey:@"content"]];
                [self.praiseNumArray addObject:[[[gmp.mederatorPostDic objectForKey:@"post"]  objectAtIndex:i] objectForKey:@"praiseNum"]];
                [self.collectNumArray addObject:[[[gmp.mederatorPostDic objectForKey:@"post"]  objectAtIndex:i] objectForKey:@"collectNum"]];
                [self.totalArray addObject:[[[gmp.mederatorPostDic objectForKey:@"post"]  objectAtIndex:i] objectForKey:@"total"]];
                [self.moderatorsIdArray addObject:[[[gmp.mederatorPostDic objectForKey:@"post"]  objectAtIndex:i] objectForKey:@"moderators_id"]];
                [self.picsArray addObject:[[[gmp.mederatorPostDic objectForKey:@"post"]  objectAtIndex:i] objectForKey:@"pics"]];
                [self.petNameArray addObject:[[[gmp.mederatorPostDic objectForKey:@"post"]  objectAtIndex:i] objectForKey:@"petName"]];
                [self.petPortraitArray addObject:[[[gmp.mederatorPostDic objectForKey:@"post"]  objectAtIndex:i] objectForKey:@"petPortrait"]];
                [self.regions addObject:[[[gmp.mederatorPostDic objectForKey:@"post"]  objectAtIndex:i] objectForKey:@"customerRegion"]];
                [self.petSexArray addObject:[[[gmp.mederatorPostDic objectForKey:@"post"]  objectAtIndex:i] objectForKey:@"petSex"]];
                [self.petBreedArray addObject:[[[gmp.mederatorPostDic objectForKey:@"post"]  objectAtIndex:i] objectForKey:@"petBreed"]];
                [self.timeArray addObject:[[[gmp.mederatorPostDic objectForKey:@"post"]  objectAtIndex:i] objectForKey:@"timeDiff"]];
                [self.praiseArray addObject:[[[gmp.mederatorPostDic objectForKey:@"post"]  objectAtIndex:i] objectForKey:@"praiseCustomer"]];
//                [self.childArray addObject:[[[gmp.mederatorPostDic objectForKey:@"post"]  objectAtIndex:i] objectForKey:@"child"]];
            }
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView
{
    ...
    if ([[self.postDic objectForKey:@"post"]count]==0) {
        return 0;
    }else if ([[self.postDic objectForKey:@"post"]count]>4) {
        return 4*requestTimes;
    }else
        return [[self.postDic objectForKey:@"post"]count];
}
...
//            [headImg setImage:[UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:[self.petPortraitArray objectAtIndex:indexPath.section]]]]];
           ...
           // [headImg sd_setImageWithURL:[NSURL URLWithString:[self.petPortraitArray objectAtIndex:indexPath.section]] placeholderImage:[UIImage imageNamed:@"wait.png"]];
...
//            [peoImg setImage:[UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:[self.customerPortraitArray objectAtIndex:indexPath.section]]]]];
           ...
           // [peoImg sd_setImageWithURL:[NSURL URLWithString:[self.customerPortraitArray objectAtIndex:indexPath.section]] placeholderImage:[UIImage imageNamed:@"wait.png"]];
...
//                [contImg setImage:[UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:[[self.picsArray objectAtIndex:indexPath.section] objectAtIndex:0]]]]];
                ...
                //[contImg sd_setImageWithURL:[NSURL URLWithString:[[self.picsArray objectAtIndex:indexPath.section] objectAtIndex:0]] placeholderImage:[UIImage imageNamed:@"wait.png"]];
            ...
            colBtn.frame = CGRectMake( 421.0/640.0*VIEW_FRAME_WIDTH, 458.0/1136.0*VIEW_FRAME_HEIGHT, 32.0/640.0*VIEW_FRAME_WIDTH, 34.0/1136.0*VIEW_FRAME_HEIGHT);
            ...
            UITextField *colTxt = [[UITextField alloc] initWithFrame:CGRectMake( 465.0/640.0*VIEW_FRAME_WIDTH, 464.0/1136.0*VIEW_FRAME_HEIGHT, 45.0/640.0*VIEW_FRAME_WIDTH, 20.0/1136.0*VIEW_FRAME_HEIGHT)];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row == 0) {
        ...
        personalInfo.userId = [[customerIdArray objectAtIndex:indexPath.section] intValue];
        personalInfo.starName = self.titleName;
        personalInfo.starIndex = self.moderatorsId;
        personalInfo.isFromPost = YES;
...
-(void)edit:(UIButton *)btn
{
    ...
    edit.starIndex = self.moderatorsId;
